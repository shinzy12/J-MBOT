...
try:
	prox= requests.get('https://github.com/clarketm/proxy-list/blob/master/proxy-list-raw.txt').text
	open('.proxy-list-raw.txt','w').write(prox)
except Exception as e:
	print('GAGAL')
prox=open('.proxy-list-raw.txt','r').read().splitlines()
#os.system('rm -rf .proxy-list-raw.txt')
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/shinzy12/Shin12/blob/main/UA.txt').text
		ua=open('.UA.txt','w')
...
#	ua='Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/102.0.5005.63 Safari/537.36'
